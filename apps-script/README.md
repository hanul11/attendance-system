...
- `HolidaySync.gs`: Google Calendar 대한민국 공휴일 동기화와 다일 공휴일 해석
- `AttendanceRequests.gs`: 연차 확인 후보와 근태 수정 요청 로그 처리
...
- 공휴일과 주말은 출퇴근 미등록 연차 후보에서 제외합니다.
- 연차 후보는 직원 확인용이며 연차를 자동 차감하지 않습니다.
- 근태 수정 요청은 기존 `근태 로그`에 기록하고 `근태현황`은 자동 수정하지 않습니다.

## 공휴일 자동 연동

1. 실행 계정의 Google Calendar에 `대한민국의 공휴일` 캘린더를 추가합니다.
2. Apps Script 편집기에서 `syncKoreanHolidaysToSheet`를 한 번 실행하고 Calendar 읽기 권한을 승인합니다.
3. 결과가 `공휴일` 시트 A:C의 3행 이후에 추가되는지 확인합니다. 기존 수동 행은 삭제하거나 덮어쓰지 않습니다.
4. `installDailyHolidaySyncTrigger`를 한 번 실행합니다. 동기화는 매일 오전 1시대에 실행됩니다.
5. Apps Script 웹앱의 새 버전을 배포해 직원·관리자 화면에 변경사항을 반영합니다.

Google의 시간 기반 트리거는 지정 시각의 정확한 분이 아니라 해당 시간대 안에서 실행될 수 있습니다.
